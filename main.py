...
def senddm_limit_handled(cursor):
...
`python main.py --dm --cc us --token bitcoin --limit 100`
  """
    ...
    parser.add_argument('--populate', action="store_true",
                        help="Update follower db locally")
    parser.add_argument('--partial', action="store_true",
                        help="If you already had your local db, and want to update it")

    # Arguments for filtering your followers and sending dms if you want
    parser.add_argument('--dm', action="store_true",
                        help="Send DM's, you run this after you update your follower db locally")
    parser.add_argument('--forreals', action="store_true",
                        help="Only when you add forreals flag you will send dms")
    parser.add_argument('--cc', type=str, default="",
                        help="Filter your followers based on country code")
    parser.add_argument('--token', type=str, default="",
                        help="Filter your followers based on a search token")
    parser.add_argument('--days', type=int, default=0,
                        help="Get followers who are active within the last n number of days")
    parser.add_argument('--limit', type=int, default=10,
                        help="Get the top n followers who have the most no of followers")
    ...
    if args.populate:
        ...
        print("Estimated time for the download to complete based on twitter api rate limits is %f hours" % (
            time_in_hours))
        ...
        if args.cc != '':
            ...
            print('Total no of followers who are from the country %s are %d' %
                  (args.cc, len(followers_json)))
        if args.token != '':
            followers_json = token_filter(followers_json, args.token)
            print('Total no of followers based on the token %s are %d' %
                  (args.token, len(followers_json)))
        if args.days != 0:
            followers_json = lastseen_filter(followers_json, args.days)
            print('Total no of followers who are last seen within %d days are %d' % (
                args.days, len(followers_json)))
        followers_json = top_n_filter(followers_json, args.limit)

        print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
        print("No of followers after all the filtering is done is %d" %
              len(followers_json))
